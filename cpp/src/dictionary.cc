...
namespace fasttext {
    ...
    void Dictionary::split(
        ...
        for (size_t i = 0; i < word.size(); ) {
            ...
            } else if (isalpha(word[i]) && word[i] != args_-> emptyjschar[0]) {
                jasos.push_back(word[i++]);
            } else if (word[i] == args_->hanjaBracket[0]) {
                ++i;
                std::string hanja;
                hanja.append(BOW);
                while (i < word.size() && (word[i] != args_->hanjaBracket[1])) {
                    hanja.push_back(word[i++]);
                    while (i < word.size() && (word[i] & 0xC0) == 0x80) {
                        hanja.push_back(word[i++]);
                    }
                }
                hanja.append(EOW);
                hanjas.push_back(hanja);
                ++i;
            } else {
    ...
    void Dictionary::computeSubwords(
        ...
        for (size_t k = 0; k < hanjas.size(); ++k) {
            ...
            for (size_t i = 0, n = 0; i < hanja.size(); ++i) {
                if ((hanja[i] & 0xC0) == 0x80) {
                    continue;
                }
                std::string ngram;
                for (size_t j = i, n = 1; j < hanja.size() && n <= args_->maxhn; ++n) {
                    ngram.push_back(hanja[j++]);
                    while (j < hanja.size() && (hanja[j] & 0xC0) == 0x80) {
                        ngram.push_back(hanja[j++]);
                    }
                    if (n >= args_->minhn && !(n == 1 && (i == 0 || j == hanja.size()))) {
        ...
        for (size_t i = 0, n_jamos = 0; i < wordkr.size(); i++) {
            if ((wordkr[i] & 0xC0) == 0x80) {
                continue;
            }
            n_jamos++;
            if (isalpha(wordkr[i-1]) && wordkr[i-1] != args_->emptyjschar[0]) {
                n_jamos += 2;
            }
            std::string jamogram;
            for (size_t j = i, n = 1; j < wordkr.size() && n <= args_->maxjn; n++) {
                jamogram.push_back(wordkr[j++]);
                while (j < wordkr.size() && (wordkr[j] & 0xC0) == 0x80) {
                    jamogram.push_back(wordkr[j++]);
                }
            ...
            std::string chargram;
            if (n_jamos % 3 == 2) {
                for (size_t j = i, n = 1, n_jamos = 1; j < wordkr.size(); n++) {
                    if (isalpha(wordkr[j]) && wordkr[j] != args_->emptyjschar[0]) {
                        n_jamos += 2;
                    }
